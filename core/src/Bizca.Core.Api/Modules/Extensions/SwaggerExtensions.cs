namespace Bizca.Core.Api.Modules.Extensions
{
    ...
    {
        internal static void AddSwaggerSecurity(this SwaggerGenOptions opts,
            IEnumerable<SwaggerSecurityDefinitionModel> security)
        {
            if (security is null)
                return;

            SwaggerSecurityDefinitionModel[] securityArray =
                security as SwaggerSecurityDefinitionModel[] ?? security.ToArray();
            ...
            foreach (SwaggerSecurityDefinitionModel securityDefinition in securityArray)
            {
                ...
                opts.AddSecurityDefinition(securityDefinition.DisplayName,
        ...
        internal static void AddSwaggerStsSecurity(this SwaggerGenOptions opts, SwaggerStsSecurityModel security)
        {
            ...
            Dictionary<string, string> scopesDictionary = security.Scopes.Aggregate(new Dictionary<string, string>(),
                (agg, curr) =>
                {
                    agg.Add(curr, string.Empty);
                    return agg;
                });
            opts.AddSecurityDefinition("oauth2",
                new OpenApiSecurityScheme
                {
                    Name = "Authorization",
