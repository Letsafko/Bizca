namespace Bizca.Core.Domain
{
    ...
    public sealed class Notification
    {
        ...
        public IReadOnlyDictionary<string, string[]> Errors =>
            errorMessages.ToDictionary(item => item.Key, item => item.Value.ToArray());

        public void Add(IDictionary<string, string[]> errors)
        {
            foreach (KeyValuePair<string, string[]> err in errors) Add(err);
        }

        public void Add(KeyValuePair<string, string[]> error)
        {
            string key = error.Key.ToLower();
            if (!errorMessages.ContainsKey(key)) errorMessages[key] = new List<string>();
