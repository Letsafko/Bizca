namespace Bizca.User.WebApi.UseCases.V1.ConfirmChannelCode
{
    ...
    public sealed class UsersController : ControllerBase
    {
        ...
        {
            ConfirmChannelCodeCommand command = GetConfirmationCommand(userId, partnerCode, input);
            await _processor.ProcessCommandAsync(command).ConfigureAwait(false);
            return _presenter.ViewModel;
        }

        private ConfirmChannelCodeCommand GetConfirmationCommand(string userId, string partnerCode, ConfirmChannelCode input)
        {
