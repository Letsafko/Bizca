...
return
    select 
        ...
        max(case when channelId = 3 then value end) as whatsapp,
        max(case when channelId = 3 then convert(int, active) end) as whatsappActive,
        max(case when channelId = 3 then convert(int, confirmed) end) as whatsappConfirmed,
        max(case when channelId = 4 then value end) as messenger,
        max(case when channelId = 4 then convert(int, active) end) as messengerActive,
        max(case when channelId = 4 then convert(int, confirmed) end) as messengerConfirmed
